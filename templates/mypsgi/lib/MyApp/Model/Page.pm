...
sub entries {
	...
	$self->{_entries} ||= do {
		...
		<?= $_->{module} ?>::Model->select($query, $self->{hash}, $self->{array}, $self->{name});
	};
}
...
sub total {
	...
	defined $self->{_total} ? $self->{_total} : $self->{_total} = do {
		...
		<?= $_->{module} ?>::Model->select($query, $self->{hash}, $self->{array}, $self->{name})->[0]->get_column('total')
	};
}
