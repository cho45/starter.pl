package <?= $_->{module} ?>::Model::Row::Entry;
use strict;
use warnings;
...
sub update_tags {
	...
	for my $name (@$applied) {
		<?= $_->{module} ?>::Model->create('tag', { name => $name, entry_id => $self->id });
	}
	...
	for my $name (@$removed) {
		<?= $_->{module} ?>::Model->delete('tag', { name => $name, entry_id => $self->id });
	}
}

sub tags {
	my ($self) = @_;
	my $tags = <?= $_->{module} ?>::Model->select(q{
		SELECT * FROM tag
